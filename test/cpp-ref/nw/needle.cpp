...
class pragma107_omp_parallel_hclib_async;
#else
static void pragma107_omp_parallel_hclib_async(void *____arg, const int ___iter0);
#endif

#ifdef OMP_TO_HCLIB_ENABLE_GPU
class pragma159_omp_parallel_hclib_async;
#else
...
domain[0].tile = -1;
#ifdef OMP_TO_HCLIB_ENABLE_GPU
hclib::future_t *fut = hclib::forasync_cuda((blk) - (0), pragma107_omp_parallel_hclib_async(), hclib::get_closest_gpu_locale(), NULL);
fut->wait();
#else
...
domain[0].tile = -1;
#ifdef OMP_TO_HCLIB_ENABLE_GPU
hclib::future_t *fut = hclib::forasync_cuda(((max_cols - 1) / 16) - (blk - 1), pragma159_omp_parallel_hclib_async(), hclib::get_closest_gpu_locale(), NULL);
fut->wait();
#else
