...
static void pragma41_hclib_async(void *____arg);
...
hclib_async(pragma31_hclib_async, new_ctx, NO_FUTURE, ANY_PLACE);
 ...
 hclib_end_finish(); hclib_start_finish(); ;
            ...
            for (block_x = 0; block_x < max_blocks_x; block_x++) {
                for (block_y = 0; block_y < max_blocks_y; block_y++) {
 { 
pragma41 *new_ctx = (pragma41 *)malloc(sizeof(pragma41));
new_ctx->block_x = block_x;
new_ctx->block_y = block_y;
new_ctx->nx_ptr = &(nx);
new_ctx->ny_ptr = &(ny);
...
new_ctx->unew__ptr = &(unew_);
new_ctx->block_size_ptr = &(block_size);
hclib_async(pragma41_hclib_async, new_ctx, NO_FUTURE, ANY_PLACE);
 } ;
                }
            }
