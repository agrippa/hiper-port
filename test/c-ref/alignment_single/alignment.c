...
static void pairalign457_hclib_async(void *arg) {
    pairalign457 *ctx = (pairalign457 *)arg;
    int i; i = ctx->i;
    int n; n = ctx->n;
    int m; m = ctx->m;
    int si; si = ctx->si;
         ...
         for (sj = si + 1; sj < nseqs; sj++)
         {
            ...
            if ( n == 0 || m == 0 ) {
               bench_output[si*nseqs+sj] = (int) 1.0;
                ...
                { 
pairalign457 *ctx = (pairalign457 *)malloc(sizeof(pairalign457));
ctx->i = i;
ctx->n = n;
ctx->m = m;
ctx->si = si;
...
ctx->matptr = matptr;
hclib_async(pairalign457_hclib_async, ctx, NO_FUTURE, NO_PHASER, ANY_PLACE);
 }  // end task
            } // end if (n == 0 || m == 0)
         } // for (j)
