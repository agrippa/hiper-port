...
static void main_entrypoint(void *arg) {
    ...
    int argc = ctx->argc;
    char ** argv = ctx->argv;
    long long program_start_time = ctx->program_start_time;
    int num_frames = ctx->num_frames;
    char * video_file_name = ctx->video_file_name;
    avi_t * cell_file = ctx->cell_file;
    int i = ctx->i;
    int j = ctx->j;
    int * crow = ctx->crow;
    int * ccol = ctx->ccol;
    int pair_counter = ctx->pair_counter;
    int x_result_len = ctx->x_result_len;
    int Iter = ctx->Iter;
    int ns = ctx->ns;
    int k_count = ctx->k_count;
    int n = ctx->n;
    MAT * cellx = ctx->cellx;
    MAT * celly = ctx->celly;
    MAT * A = ctx->A;
    double * GICOV_spots = ctx->GICOV_spots;
    double * t = ctx->t;
    double * G = ctx->G;
    double * x_result = ctx->x_result;
    double * y_result = ctx->y_result;
    double * V = ctx->V;
    double * QAX_CENTERS = ctx->QAX_CENTERS;
    double * QAY_CENTERS = ctx->QAY_CENTERS;
